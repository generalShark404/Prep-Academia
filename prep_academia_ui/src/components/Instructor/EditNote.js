...
function EditNoteAndSummary() {
  ...
  const updateNote = (e) => {
    ...
    _noteData.append("topic", note_id);
    ...
    if(!noteTitle || !content){
      ...
      try {
        axiosInstance
        .put(`instructor/update/note/${note_id}`, _noteData, {
          headers: {
          ...
          .then((res) => {
              if(res.status == 200 || res.status == 201){
                  Swal.fire({
                    title: "Success",
                    icon: "success",
                    html:`<p class="fs-2">Note updated successfully !</p>`,
                    timer: "2000",
                  });
              };
          }).then((err) => {});
  ...
  useEffect(() => {
    axiosInstance.get(`instructor/get/note/${note_id}`)
    .then((res) => {
        console.log(res)
...
export default  EditNoteAndSummary;